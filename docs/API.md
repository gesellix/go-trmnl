...
| `Refresh-Rate`                       | no       | Interval the device currently sleeps for. Recorded in device logs, but deliberately **not** applied to the device's configured refresh rate: the firmware also reports its retry and fast-poll backoffs here (its ladder starts at 15 seconds), and storing one would be handed straight back in the next `refresh_rate` response, leaving the device awake every few seconds. The schedule is owned by the admin UI. |
