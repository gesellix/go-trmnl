...
# - It installs a systemd unit for trmnld.
...
download_url_for() {
  local asset="$1"
  echo "https://github.com/gesellix/go-trmnl/releases/download/${VERSION}/trmnld-${VERSION}-${asset}"
}
...
ensure_dirs() {
  ...
  chmod 0755 "${CONFIG_DIR}" "${TRMNL_DATA_DIR}"
}
...
download_binary() {
  ...
  if command -v curl >/dev/null 2>&1; then
    curl -fsSL -o "${tmp}/trmnld" "${url}"
  else
    wget -qO "${tmp}/trmnld" "${url}"
  fi
  ...
  install -m 0755 "${tmp}/trmnld" "${BIN_PATH}"
